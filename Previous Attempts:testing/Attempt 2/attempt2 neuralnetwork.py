...
class NeuralNetwork:
    ...
    def feedForward(self,inputs):
        """
        Passes Inputs and returns outputs
        """
    ...
    def train(self,inputs,actualOutputs):
        ...
        errorSignals = NeuralNetwork.outputErrorSignal(activationLayers,actualOutputs)
        for currentLayerNum in range(len(self.layers)-1,0,-1): #Starts at second last layer goes to second layer
            if currentLayerNum == (len(self.layers) - 1):
                self.outputLayerBackPropogation(currentLayerNum,errorSignals,activationLayers)
            else:
                errorSignals = self.hiddenLayerBackPropogation(currentLayerNum,errorSignals,activationLayers)
        self.applyDelta()

    def backPropogation(self,neuronIndex,weightIndex,currentLayerNum,errorSignal,prevActivationDeriv):
        """
        Basic Chain Rule step of finding derivative ie direction to move
        Gradient Descent
        Partial Derivates: [Error Signal->(nonLin Deriv and Error Deriv)] and previousLayerActivation
        """
        neuronWeightDerivative = errorSignal * prevActivationDeriv #Derivative
        delta = neuronWeightDerivative * self.learningRate #Delta Change
        if (currentLayerNum,neuronIndex,weightIndex) in self.delta: self.delta[(currentLayerNum,neuronIndex,weightIndex)] += delta
        else: self.delta[(currentLayerNum,neuronIndex,weightIndex)] = delta

    def outputLayerBackPropogation(self,currentLayerNum,errorSignals,activationLayers):
        for neuronIndex in range(self.layerInformation[currentLayerNum]):
            errorSignal = errorSignals[neuronIndex]
            for weightIndex in range(self.layerInformation[currentLayerNum-1]):
                prevActivationDeriv = activationLayers[currentLayerNum-1][weightIndex]
                self.backPropogation(neuronIndex,weightIndex,currentLayerNum,errorSignal,prevActivationDeriv)

    def hiddenLayerBackPropogation(self,currentLayerNum,errorSignals,activationLayers):
        newErrorSignals = []
        for neuronIndex in range(self.layerInformation[currentLayerNum]):
            nonlinDerivative = NeuralNetwork.nonlin(activationLayers[currentLayerNum][neuronIndex],derivative=True)
    ...
    def hiddenErrorDerivative(self,neuronIndex,currentLayerNum,errorSignals):
        ...
        """
        totalErrorDerivative = 0
        nextLayer = self.layers[currentLayerNum+1]
        for index,currentNeuron in enumerate(nextLayer.neurons):
            weightDerivative = currentNeuron.weights[neuronIndex]
            errorSignal = errorSignals[index]
            totalErrorDerivative += weightDerivative * errorSignal
        return totalErrorDerivative
    
    @staticmethod
    def outputErrorSignal(activationLayers,actualOutputs):
    ...
    def createNNFile(self,fileName):
        """
        Creates a file that holds NN information
        """
    ...
    def applyDelta(self):
        ...
        for indexes in self.delta:
            ...
            self.delta[indexes] = 0 #Resets delta
...
class Neuron:
    ...
    def weightsAndBiases(self,previousLayerSize):
        """
        Sets Weights and Biases randomly with knowledge of previous layers
        """
    ...
    def getActivation(self,activation,neuronIndex):
        """
        Gets activation of neuron for one weight
        """
...
class NetworkLayer:
    ...
    def getActivationLayer(self,activations):
        """
        Gets new activation layer from given inputs
        """
        newActivations = []
...
if __name__ == '__main__':
    ...
    if test:
        ...
        outputs = [0.7]
        iterations = 100
        print('Initial',NN.feedForward(inputs)[-1])

        for i in range(iterations):
            NN.train(inputs,outputs)

        print('Final',NN.feedForward(inputs)[-1])
    else:
